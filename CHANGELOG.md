...
- Initial Samsung The Frame Art Mode proof of concept with image preflight,
  read-only diagnostics, explicit upload/display commands, and mocked tests.
- NAS deployment foundation: Docker runtime, localhost-only host port,
  tailnet nginx template, NAS-backed catalogue, 4K rendering, curator API,
  operation audit, and rollback state.
